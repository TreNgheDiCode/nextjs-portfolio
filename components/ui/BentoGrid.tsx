...
export const BentoGridItem = ({
  ...
  const leftLists = ["NextJS", "ASP.NET", "API"];
  const rightLists = ["CI/CD", "TypeScrypt", "C#"];

  const [copied, setCopied] = useState(false);

  const handleCopy = () => {
    const text = "cigpbubu@gmail.com";
            ...
            <div className="absolute -right-3 flex w-fit gap-1 lg:-right-2 lg:gap-5">
              ...
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-8">
                {leftLists.map((item, i) => (
                  <span
                    key={i}
                    className="rounded-lg bg-[#10132E] px-3 py-2 text-center text-xs opacity-50 lg:text-base lg:opacity-100"
                  >
                    {item}
                  </span>
                ))}
